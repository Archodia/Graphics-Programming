...
function draw() {
	...
	//scale down currImg per instructions to quarter of size:
	currImg.resize(currImg.width/4, currImg.height/4);
	//running the blur filter on the currImg per instructions to reduce notes 'noise':
	currImg.filter(BLUR, 3);

    diffImg = createImage(video.width, video.height);
	//scale down diffImg per instructions to quarter of size:
	...
	// grid to be called last, per instructions:
	grid.run(diffImg);
}
